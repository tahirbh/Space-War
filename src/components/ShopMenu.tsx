...
export function ShopMenu({ onContinue }: ShopMenuProps) {
  ...
  return (
    <div className="absolute inset-0 bg-[#0A0A15]/95 z-40 flex flex-col items-center p-4 sm:p-8 animate-in fade-in duration-500 overflow-hidden">
      {/* Header - Fixed */}
      <div className="text-center mb-4 sm:mb-8 mt-2 sm:mt-0 shrink-0">
        <h2 className="text-xl sm:text-5xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-600 mb-0.5 sm:mb-2"
            style={{ fontFamily: 'Orbitron, sans-serif' }}>
          ARMORY
        </h2>
        <div className="flex items-center justify-center gap-2 text-yellow-400">
          <ShoppingCart className="w-3 h-3 sm:w-5 sm:h-5" />
          <span className="text-xs sm:text-xl font-mono tracking-widest">{coins.toString().padStart(6, '0')}</span>
        </div>
      </div>

      {/* Scrollable Content Area */}
      <div className="flex-1 w-full max-w-4xl overflow-y-auto scrollbar-hide pb-20 sm:pb-32">
        {/* Ship Preview / Weapon Status */}
        <div className="w-full bg-white/5 border border-white/10 rounded-xl sm:rounded-2xl p-2 sm:p-6 mb-3 sm:mb-8 flex flex-col sm:flex-row items-center gap-2 sm:gap-8 backdrop-blur-xl shrink-0">
          <div className="hidden sm:flex w-16 h-16 sm:w-32 sm:h-32 flex-col items-center justify-center bg-cyan-500/10 rounded-full border border-cyan-500/30 shadow-[0_0_20px_rgba(0,212,255,0.1)] shrink-0">
          ...
          <div className="flex-1 grid grid-cols-4 gap-1 sm:gap-4 w-full">
        ...
        {/* Shop Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-4 w-full shrink-0">
          {SHOP_ITEMS.map((item) => (
            <button
              key={item.id}
              ...
              className={cn(
                "group relative flex items-center gap-2 sm:gap-4 p-2.5 sm:p-4 rounded-xl border transition-all duration-300",
                "bg-white/5 border-white/10 hover:border-yellow-400/50 hover:bg-white/10",
                "disabled:opacity-50 disabled:cursor-not-allowed disabled:grayscale"
              )}
            >
              <div className="w-8 h-8 sm:w-12 sm:h-12 flex items-center justify-center bg-yellow-400/10 rounded-lg text-yellow-400 group-hover:scale-110 transition-transform shrink-0">
                {item.icon}
              </div>
              <div className="flex-1 text-left min-w-0">
                <h3 className="text-xs sm:text-base text-white font-bold truncate">{item.name}</h3>
                <p className="text-white/40 text-[8px] sm:text-xs leading-tight line-clamp-1">{item.desc}</p>
              </div>
              <div className="text-right shrink-0">
                <div className="text-yellow-400 font-mono font-bold text-xs sm:text-base">{item.cost} G</div>
                <div className="text-[7px] sm:text-[10px] text-white/20 uppercase tracking-widest">
...
function StatBox({ label, value, color }: { label: string, value: number, color: string }) {
    return (
        <div className="bg-black/40 p-1.5 sm:p-3 rounded-lg border border-white/5 flex flex-col items-center sm:items-start">
            <div className="text-[7px] sm:text-[10px] text-white/40 uppercase tracking-tighter mb-0.5 sm:mb-1">{label}</div>
            <div className={cn("text-[10px] sm:text-xl font-mono font-bold leading-none", color)}>
